...
module RedmineDevIntegration
  class BitbucketDeploymentProcessor
    ...
    def call(external_provider_event)
      ...
      return false unless external_provider_event.event_type == 'repo:deployment'

      repository = external_repository_for(payload)
      return false unless repository

      deployment = payload['deployment'] || {}
      provider_deployment_id = deployment['uuid'].to_s
      return false if provider_deployment_id.blank?

      environment_name = deployment.dig('environment', 'name').presence ||
                         (deployment['environment'].is_a?(String) ? deployment['environment'].presence : nil) ||
                         'unknown'
      ...
      raw_status = deployment.dig('state', 'name').presence
      external_deployment.environment_url = deployment.dig('release', 'url').presence || deployment['url']
      external_deployment.status = normalized_status(raw_status, deployment)
      external_deployment.sha = deployment.dig('release', 'commit')
      external_deployment.ref = deployment.dig('release', 'name')
      external_deployment.branch_name = deployment.dig('release', 'name')
      external_deployment.description = deployment['comment'].presence || deployment['description']
      external_deployment.creator_login = deployment.dig('deployer', 'username').presence || deployment.dig('deployer', 'display_name')
      external_deployment.started_at = time_value(deployment['started_on'] || deployment['created_on'])
      external_deployment.completed_at = time_value(deployment['completed_on']) if terminal_status?(raw_status, deployment)
      external_deployment.last_event_at = time_value(deployment['updated_on'] || deployment['completed_on'] || external_provider_event.created_at)
    ...
    def normalized_status(state, deployment = {})
      case state.to_s
      when 'PENDING'
        'pending'
      when 'IN_PROGRESS'
        'in_progress'
      when 'COMPLETED'
        result = deployment.dig('state', 'result', 'name').to_s
    ...
    def terminal_status?(state, deployment = {})
      normalized = normalized_status(state, deployment)
    ...
    def associate_release(deployment, payload)
      deployment_data = payload['deployment'] || {}
      ref = deployment_data.dig('release', 'name')
